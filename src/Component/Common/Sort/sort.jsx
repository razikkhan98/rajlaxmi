...
const SortDropdown = ({ onSortChange }) => {
  ...
  const handleChange = (id) => {
    setSelected(id);
    onSortChange(id); // Notify the parent to sort
  };
  ...
  return (
    <div className="d-flex flex-column align-items-end">
      ...
      <div className="dropdown">
        ...
        {/* Dropdown Menu */}
        <ul
          className="dropdown-menu sort-menu py-3 px-2"
          style={{ backgroundColor: "#fdf7de" }}
        >
          <li className="filter-heading fw-medium text-heading pb-3 px-2">
            SORT BY
          </li>
          {sortOptions?.map((option) => (
            // <li
            //   key={option?.id}
            //   className="d-flex align-items-center my-2 justify-content-between px-2"
            // >
            //   <label className="form-check-label text-color-dark-ashy-blue">
            //     {option?.label}{" "}
            ...
            <li
              ...
              className="d-flex align-items-center my-2 justify-content-between px-2"
            >
              <label className="form-check-label text-color-dark-ashy-blue d-flex w-100 justify-content-between">
                <div>{option?.label}{" "}
                <span className="sort-red-text">{option?.text}</span></div>
                <input
                  type="checkbox"
                  className="form-check-input me-2"
                  checked={selected === option?.id}
